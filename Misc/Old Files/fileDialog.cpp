...
void setupInternalFilter (char *internalFilter, char *userFilter) {
	...
	for(int index=strlen(internalFilter);index>=0;index--) {
		if(internalFilter[index] == '|') {
			internalFilter[index] = '\0';
		}
	}
}
...
void setupQueryStructure(OPENFILENAME &queryStructure, char * filter, char * initialFileName) {
	...
	strcpy(filenameBuffer, initialFileName);
	static char fileFilter[1024];
	setupInternalFilter(&fileFilter[0], filter);
	if(lastDirectory [0] == '\0') {
		_getcwd(lastDirectory, MAX_PATH);
	}
...
char *promptForReadFile (DirectoryChoice choice, char *filter, char *initialFileName = "") {
	useDirectory (choice);
	OPENFILENAME query;
	setupQueryStructure(query, filter, initialFileName);      
